...
# Determine node suffix based on role
if [[ "$ROLE" == "iran" ]]; then
    NODE_SUFFIX="Client"
else
    NODE_SUFFIX="Server"
...
for i in "${!PORTS[@]}"; do
    ...
    chain="chain$((i+1))"
    if $USE_MUX; then
        echo "    { \"name\": \"$chain\", \"type\": \"Mux${NODE_SUFFIX}\", \"settings\": {}, \"next\": \"${chain}m\" }," >> config.json
        chain="${chain}m"; CHAIN_NODES+=("Mux")
    fi
    if [[ "$METHOD" == "half" ]]; then
        type="HalfDuplex${NODE_SUFFIX}"
    else
        type_name=$(echo "$METHOD" | sed 's/-//g')
        method_pascal=$(tr '[:lower:]' '[:upper:]' <<< ${type_name:0:1})${type_name:1}
        type="${method_pascal}${NODE_SUFFIX}"
    fi
    echo "    { \"name\": \"$chain\", \"type\": \"$type\", \"settings\": {}, \"next\": \"${chain}o\" }," >> config.json
    chain="${chain}o"; CHAIN_NODES+=("$METHOD")
    if $USE_OBFS; then
        echo "    { \"name\": \"$chain\", \"type\": \"Obfuscator${NODE_SUFFIX}\", \"settings\": {\"method\": \"xor\", \"xor_key\": \"123\"}, \"next\": \"${chain}t\" }," >> config.json
        chain="${chain}t"; CHAIN_NODES+=("Obfs")
    fi
    if $USE_TLS && [[ "$METHOD" != "tls" ]]; then
        echo "    { \"name\": \"$chain\", \"type\": \"Tls${NODE_SUFFIX}\", \"settings\": {}, \"next\": \"${chain}t2\" }," >> config.json
